...
def ridge_projected(stim, resp, alpha, singcutoff=1e-10, normalpha=False, y_projection="pca", projection_map_y=None, logger=ridge_logger):
    """Uses ridge regression to find a linear transformation of [stim] that approximates
    [resp]. The regularization parameter is [alpha].
    ...
    if y_projection =='I':
        return ridge(stim, resp, alpha, singcutoff, normalpha, logger)
...
def ridge_corr_with_projection(Rstim, Pstim, Rresp, Presp, alphas, normalpha=False, corrmin=0.2,
    ...
    ## Precompute some products for speed
    # downproject the Rresp to regularize it. Replace Rresp, Presp with the downprojected version.
    Rresp_projected = projection_map_y.transform(Rresp)

    UR = np.dot(U.T, Rresp_projected) ## Precompute this matrix product for speed (property of the projected Y variable)
    PVh = np.dot(Pstim, Vh.T) ## Precompute this matrix product for speed (property of the X variable)
    
    # Properties of the response space
    zPresp = zs(Presp) 
    Prespvar_actual = Presp.var(0)
    Prespvar = (np.ones_like(Prespvar_actual) + Prespvar_actual) / 2.0
    ...
    for na, a in zip(nalphas, alphas):
        ...
        pred_projected = np.dot(mult_diag(D, PVh, left=False), UR) ## Best (1.75 seconds to prediction in test)
        pred = up_projection_map(pred_projected)

        if use_corr:
            Rcorr = (zPresp * zs(pred)).mean(0) # This is the correlation in response space
        else:
            ## Compute variance explained
            resvar = (Presp - pred).var(0)
            Rsq = 1 - (resvar / Prespvar)
...
def bootstrap_ridge_with_y_projection(Rstim, Rresp, Pstim, Presp, alphas, nboots, chunklen, nchunks,
                    corrmin=0.2, joined=None, singcutoff=1e-10, normalpha=False, single_alpha=False,
                    use_corr=True, return_wt=True, y_projection='I', projection_map_y=None, up_projection_map=None, logger=ridge_logger):
    """Uses ridge regression with a bootstrapped held-out set to get optimal alpha values for each response.
    [nchunks] random chunks of length [chunklen] will be taken from [Rstim] and [Rresp] for each regression
    run.  [nboots] total regression runs will be performed.  The best alpha value for each response will be
    averaged across the bootstraps to estimate the best alpha for that response.
    ...
    if return_wt:
        ...
        pred_projected = np.dot(Pstim, wt)
        pred = up_projection_map(pred_projected)
